...
async function setupDatabase() {
  ...
  try {
    ...
    console.log(`Database '${process.env.DB_NAME || 'cafe_db'}' created or already exists`);

    // Use the database
    await connection.query(`USE ${process.env.DB_NAME || 'cafe_db'}`);
    ...
    for (const product of products) {
      await connection.execute(
        ...
        [product.id, product.name, product.category_id, product.price, product.description, product.image_url, product.is_available, product.is_favorite]
      );
    }
    ...
    console.log('Default admin user created (phone: 09123456789, password: admin123)');
